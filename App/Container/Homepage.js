...
const Homepage = ({ navigation }) => {
    ...
    const [statusBarCollor, setStatusBarColor] = useState('#E4F0FF')
    useEffect(() => {
    ...
    const Background = () => {
        return (
            <View style={{ paddingHorizontal: 25, paddingVertical: 25, backgroundColor: '#E4F0FF' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <View>
                        <Text allowFontScaling={false} style={{ fontFamily: Fonts.FontsFamily.fontSemiBold, fontSize: Fonts.FontSize.xl, color: Colors.BLACK_COLOR }}>Rp500.000</Text>
                        <Text allowFontScaling={false} style={{
                            fontFamily: Fonts.FontsFamily.fontRegular,
                            fontSize: 12,
                            color: Colors.GRAY_DARK_COLOR
                        }}>SPE Points{' '}
                            <Text allowFontScaling={false} style={{ fontFamily: Fonts.FontsFamily.fontSemiBold, color: Colors.ORANGE_COLOR }}>1.520</Text></Text>
                    </View>
                        ...
                        {menuHeaderList.map((menuHeader, index) => {
                            return (
                                <TouchableOpacity
                                    ...
                                    <Text allowFontScaling={false} style={{ fontFamily: Fonts.FontsFamily.fontMedium, color: Colors.BLACK_COLOR, fontSize: 12, marginTop: 10 }}>{menuHeader.title}</Text>
                                </TouchableOpacity>
                            )
                        })}
                    </View>
    ...
    const Separator = () => {
        return (
            <View style={{ backgroundColor: '#E4F0FF', height: 10, width: '100%', marginVertical: 25 }} />
        )
    }
    return (
        <>
            ...
            <ParallaxScroll
                ...
                onChangeHeaderVisibility={(isVisible) => {
                    if (isVisible) {
                        setStatusBarColor('#E4F0FF')
                    } else {
                        setStatusBarColor(Colors.WHITE_COLOR)
                    }
                }}
            >
                <View style={{ backgroundColor: '#E4F0FF', flex: 1 }}>
                                            ...
                                            {outerMenuContent.map((menuHeader, index) => {
                                                return (
                                                    <TouchableOpacity
                                                        onPress={() => {
                                                            if (menuHeader.navigate !== '') {
                                                                navigation.navigate(menuHeader.navigate)
                                                            }
                                                        }}
                                                        key={'menuHeaderList' + index}
