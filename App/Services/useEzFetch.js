...
const baseUrl =
  config.developmentENV === 'stg' ? config.stgApiURL : config.apiURL;
...
const useEzFetch = (url, initialPayload = initialData) => {
  ...
  const get = async (param, headers = {}, callBack) => {
    if (__DEV__) {
      Logger(`REQ-GET-USEEZFETCH-${param}`, {}, state);
    }
    let response, err;
    try {
      ...
      if (__DEV__) {
        Logger(`DONE-GET-USEEZFETCH-${param}`, {response, err}, state);
      }
      if (callBack) {
        callBack({response, err, state});
      }
    }
    return response;
  };
  ...
  const post = async (param, data = {}, headers = {}, callBack) => {
    if (__DEV__) {
      Logger(`REQ-POST-USEEZFETCH-${param}`, {data}, state);
    }
    let response, err;
    try {
      dispatch({type: 'request'});
      ...
      if (__DEV__) {
        Logger(`DONE-POST-USEEZFETCH-${param}`, {response, err}, state);
      }
      if (callBack) {
        callBack({response, err, state});
      }
  ...
  const resolved = res => {
    if (res?.status === 200) {
      ...
      dispatch({type: 'failure', error: res?.status});
    } else {
      if (isEmpty(res.data.error)) {
        dispatch({type: 'failure', error: res.data.message});
      } else {
